...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
	...
	if(nPrevHeight <= 7200)
	{
		return 1 * COIN; //10 days for test.
	}
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue)
{
    ...
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 2)) ret += blockValue / 20; //25%
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 3)) ret += blockValue / 20; //30%
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 4)) ret += blockValue / 20; //35%
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 5)) ret += blockValue / 20; //40%
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 6)) ret += blockValue / 20; //45%
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 7)) ret += blockValue / 20; //50%
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // SGPR : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(SGPR): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(SGPR): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // SGPR : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(SGPR): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(SGPR): spork is off, skipping transaction locking checks\n");
    }
