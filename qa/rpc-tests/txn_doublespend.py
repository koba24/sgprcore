...
class TxnMallTest(BitcoinTestFramework):
    ...
    def run_test(self):
        # All nodes should start with 12,500 SGPR:
        ...
        # First: use raw transaction API to send 12400 SGPR to node1_address,
        # but don't broadcast:
        ...
        # Create two spends using 1 500 SGPR coin each
        ...
        # Node0's balance should be starting balance, plus 500SGPR for another
        ...
        # Node0's total balance should be starting balance, plus 5000 SGPR for
        # two more matured blocks, minus 12400 for the double-spend, plus fees (which are
        # negative):
        expected = starting_balance + 5000 - 12400 + fund_foo_tx["fee"] + fund_bar_tx["fee"] + doublespend_fee
        assert_equal(self.nodes[0].getbalance(), expected)
        assert_equal(self.nodes[0].getbalance("*"), expected)
