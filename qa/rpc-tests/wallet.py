...
class WalletTest (BitcoinTestFramework):
    ...
    def check_fee_amount(self, curr_balance, balance_with_fee, fee_per_byte, tx_size):
        ...
        if fee < target_fee:
            raise AssertionError("Fee of %s SGPR too low! (Should be %s SGPR)"%(str(fee), str(target_fee)))
        # allow the node's estimation to be at most 2 bytes off
        if fee > fee_per_byte * (tx_size + 2):
            raise AssertionError("Fee of %s SGPR too high! (Should be %s SGPR)"%(str(fee), str(target_fee)))
        return curr_balance
    ...
    def run_test (self):
        ...
        # Send 210 SGPR from 0 to 2 using sendtoaddress call.
        ...
        # node0 should end up with 5000 SGPR in block rewards plus fees, but
        # minus the 210 plus fees sent to node2
        assert_equal(self.nodes[0].getbalance(), 5000-210)
        ...
        # Send 100 SGPR with subtract fee from amount
        ...
        # Sendmany 100 SGPR with subtract fee from amount
